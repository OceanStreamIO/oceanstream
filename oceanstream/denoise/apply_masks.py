...
from .applying_masks_handler import apply_selected_noise_masks_and_or_noise_removal as apply_selected_masks
...
def apply_background_noise_removal(ds_processed: xr.Dataset, profiling_info: Dict, config) -> tuple[xr.Dataset, dict]:
    start_time = None
    start_cpu = None
    start_memory = None
    ...
    if config["profile"]:
        profiling_info["background noise removal"] = end_profiling(start_time, start_cpu, start_memory)
